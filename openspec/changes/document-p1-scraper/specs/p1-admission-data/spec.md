...
The system SHALL match each primary school in `schools_information.csv` to its corresponding school on sgschooling.com, and SHALL NOT silently guess an ambiguous or low-confidence match.

#### Scenario: Unambiguous match
- **WHEN** a CSV school's normalized name (or a progressively suffix-stripped form of it) exactly matches a normalized site school name
- **THEN** the CSV school is automatically matched to that site school's slug

#### Scenario: Ambiguous or unmatched CSV school
- **WHEN** a CSV school's name has no exact normalized match against any site school (e.g. due to naming differences the automated normalization cannot resolve)
